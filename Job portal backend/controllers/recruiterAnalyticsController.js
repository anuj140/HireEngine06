...
exports.getAnalytics = async (req, res) => {
  ...
  const popularJobRoles = jobs.map(job => ({
      title: job.title,
      count: applications.filter(a => a.job.toString() === job._id.toString()).length,
      percentage: totalApplications > 0 ? (applications.filter(a => a.job.toString() === job._id.toString()).length / totalApplications) * 100 : 0
  ...
  const jobPerformanceData = jobs.map(j => ({
      ...
      views: (applications.filter(a => a.job.toString() === j._id.toString()).length * (Math.random() * 5 + 2)).toFixed(0),
  }));
