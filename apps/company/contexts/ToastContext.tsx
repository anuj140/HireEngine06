...
const toastConfig = {
  ...
  error: {
    bg: 'bg-red-600',
    icon: <InformationCircleIcon className="w-6 h-6 mr-3" />,
  },
  info: {
    bg: 'bg-gray-700',
    icon: <InformationCircleIcon className="w-6 h-6 mr-3" />,
...
const Toast: React.FC<{ toast: ToastMessage; onDismiss: (id: number) => void }> = ({ toast, onDismiss }) => {
  ...
  return (
    <div
      className={`flex items-center ${config.bg} text-white p-3 rounded-lg shadow-lg animate-slide-in-right max-w-sm`}
      role="alert"
    >
