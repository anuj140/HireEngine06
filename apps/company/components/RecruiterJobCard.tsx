...
type JobAction = 'edit' | 'duplicate' | 'pause' | 'resume' | 'close' | 'notes';
...
const RecruiterJobCard: React.FC<RecruiterJobCardProps> = ({ job, onAction, userRole }) => {
  ...
  const canManageJob = userRole === 'Admin' || userRole === 'HR Manager';
    ...
    <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-200">
      ...
      <div className="mt-4 pt-4 border-t flex flex-col sm:flex-row justify-between items-start sm:items-center">
        <div className="flex flex-wrap gap-x-6 gap-y-2">
          <StatItem icon={<EyeIcon className="w-5 h-5" />} value={job.views || 0} label="Views" />
          <StatItem icon={<DocumentTextIcon className="w-5 h-5" />} value={job.applicants || 0} label="Applications" />
          <StatItem icon={<CursorClickIcon className="w-5 h-5" />} value={job.clicks || 0} label="Clicks" />
          <div className="flex items-center text-sm text-gray-600">
            <CalendarIcon className="w-5 h-5 mr-2" />
            <span className="font-bold text-dark-gray">{job.applicationDeadline ? new Date(job.applicationDeadline).toLocaleDateString() : 'N/A'}</span> End Date
          </div>
        </div>
        <Link to={`/dashboard/applicants/job/${job.id}`} className="mt-4 sm:mt-0 flex items-center px-4 py-2 bg-primary text-white rounded-full hover:bg-primary-dark transition-colors font-semibold text-sm">
          View Applicants
