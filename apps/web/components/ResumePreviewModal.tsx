...
const ResumePreviewModal: React.FC<ResumePreviewModalProps> = ({ resumeUrl, title = "Resume Preview", onClose }) => {
  useEffect(() => {
  ...
  const getFileType = (url: string) => {
      ...
      // Treat PDF and Office files as 'document' to be handled by the viewer
      return 'document';
  };
        ...
        case 'image':
             return (
                <div className="w-full h-full flex items-center justify-center bg-gray-100 overflow-auto p-4">
                    <img src={resumeUrl} alt="Resume" className="max-w-full max-h-full object-contain shadow-md" />
                </div>
             );
        case 'document':
        default:
            // Use Google Docs Viewer as a middleman for PDF, DOC, DOCX
            // embedded=true renders the file in the iframe
    ...
    <div className="fixed inset-0 bg-black/80 z-[70] flex items-center justify-center p-4 animate-fade-in" onClick={onClose}>
      <div className="bg-white rounded-xl w-full max-w-5xl h-[90vh] flex flex-col shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
        <div className="px-6 py-4 border-b flex justify-between items-center bg-white flex-shrink-0">
          <h3 className="font-bold text-lg text-dark-gray truncate pr-4">{title}</h3>
